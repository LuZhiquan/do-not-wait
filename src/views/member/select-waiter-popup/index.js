...
class SelectWaiterPopup extends Component {
  ...
  constructor(props, context) {
    super(props, context);
    this.state = this.props.memberStore.operateWaiter;
    this.props.memberStore.getWaiterList("");
  }

  handleOk = (e) => {
    this.props.memberStore.changeOperateWaiter(this.state.loginID, this.state.userName);
  ...
  render() {
    let waiters = this.props.memberStore.waiterList;
    return <Modal title="选择业务员" visible={true} maskClosable={false}  wrapClassName="vertical-center-modal"
      ...
      <div className="choose_original_people">
                ...
                this.setState({queryName: e.target.value});
		        	...
		        	<button onClick={() => {
                this.props.memberStore.getWaiterList(this.state.queryName);
              }}>查询</button>
		    	</div>
                    ...
                    if(waiters.length) {
                      return waiters.map((waiter) => {
                        return <div key={waiter.userID}
                          ...
                          onClick={() => {
                          this.props.memberStore.selectOperateWaiter(waiter.loginID);
                          this.setState({
                      ...
                      return <div className="empty-holder">暂无相关数据</div>
                    }
                  })()}
             </Scrollbars>
		    	</div>
