...
class CashPopup extends React.Component {
  constructor(props){
    super(props);
    this.state={
      price:(this.props.payAmount*1).toFixed(2)+'',
      isFirst:true,
    };
  };
// 点击数字键盘
          ...
          <div id="num-key">
            <ul>
              ...
              <li className="confirm" onClick={()=>{
                if(this.props.onOk){
                  if(parseFloat(this.state.price)===0 || this.state.price===""||this.state.price==="."){
                    message.destroy();
                    message.info("请输入金额",2);
                  }else{
                    if(this.props.offline){//线下支付
                      if(this.props.payAmount*1<this.state.price*1){
                        message.destroy();
                        message.info("你最多只需支付"+this.props.payAmount*1+"元",2);
                      }else{
